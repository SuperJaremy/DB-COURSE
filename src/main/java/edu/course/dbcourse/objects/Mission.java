...
public class Mission {
    ...
    @NonNull
    private PolicemenOnDuty policemen_on_duty;

    @NonNull
    private MachinesOnDuty machines_on_duty;
    ...
    private static final String SELECT_REINFORCEMENTS = "select call_reinforcements(%d)";
            ...
            try {
                if (rs != null && rs.next()) {
                    ...
                    Mission mission = new Mission(_id, address, akudama, rank, commander,
                            new PolicemenOnDuty(new LinkedHashSet<>()),
                            new MachinesOnDuty(new LinkedHashSet<>()));
    ...
    public boolean callReinforcements(@NonNull Database db){
        if(db.executeStatement(String.format(SELECT_REINFORCEMENTS, this.id)).isSuccess()){
            PolicemenOnDuty policemenOnDuty = PolicemenOnDuty.getPolicemenByMissionId(db, this.id);
            MachinesOnDuty machinesOnDuty = MachinesOnDuty.getMachinesByMissionId(db, this.id);
            Mission mission = Mission.getMissionById(db, this.id);
            if(machinesOnDuty != null && policemenOnDuty != null && mission != null){
                this.policemen_on_duty = policemenOnDuty;
                this.machines_on_duty = machinesOnDuty;
                this.rank = mission.getRank();
                return true;
            }
        }
        return false;
    ...
    private static Mission retrieveMission(@NonNull Database db, Database.Result res) {
        ...
        try {
            if (rs != null && rs.next()) {
                ...
                PolicemenOnDuty _policemen_on_duty = PolicemenOnDuty.getPolicemenByMissionId(db, _id);
                MachinesOnDuty _machines_on_duty = MachinesOnDuty.getMachinesByMissionId(db, _id);
    ...
    public Set<Policeman> getPolicemenOnDuty(){
        return this.policemen_on_duty.getPolicemen();
    }

    public Set<Machine> getMachinesOnDuty(){
        ...
        private static PolicemenOnDuty getPolicemenByMissionId(@NonNull Database db, int mission_id) {
            ...
            if (rs != null) {
                try {
                    ...
                    return new PolicemenOnDuty(_policemen);
                } catch (SQLException e) {
                    System.out.println(e.getMessage());
                    return null;
                }
    ...
    @AllArgsConstructor
    private static class MachinesOnDuty {
        ...
        private static MachinesOnDuty getMachinesByMissionId(@NonNull Database db,
                                                               int mission_id) {
            Database.Result res = db.executeStatement(String.format(SELECT_WHERE_MISSION_ID,
                    mission_id));
            if (!res.isSuccess())
            ...
            if (rs != null) {
                try {
                    ...
                    while (rs.next()) {
                        _machines_ids.add(rs.getInt("Machine_id"));
                    }
                    rs.close();
                    ...
                    return new MachinesOnDuty(_machines);
                } catch (SQLException e) {
                    System.out.println(e.getMessage());
                    return null;
                }
